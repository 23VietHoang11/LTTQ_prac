...
namespace LTTQ_prac
{
    public partial class Form1 : Form
    {
        ...
        {
            dgvHang.DataSource = dp.DocBang("SELECT * FROM tblHang");
        }

        private void cbChatLieu_Click(object sender, EventArgs e)
        {
            cbChatLieu.DataSource = dp.DocBang("Select * from tblChatLieu");
            cbChatLieu.ValueMember = "MaChatLieu"; //Giá trị của phần tử
            cbChatLieu.DisplayMember = "TenChatLieu";//Chuỗi hiển thị của phần tử
        }

        private void btnTimKiem_Click(object sender, EventArgs e)
        {
            //Trường hợp không chọn tiêu chí tìm sẽ lấy ra tất cả các mặt hàng
            string sql = "select MaHang, TenHang, tblHang.MaChatLieu, tblHang.TenChatLieu, DonGiaNhap, DonGiaBan, SoLuong " +
                         "from tblHang inner join tblChatLieu on " +
            ...
            if (cbChatLieu.Text != "")
                sql = sql + " and tblHang.MaChatLieu='" + cbChatLieu.SelectedValue.ToString() + "'";

            //Trình bày gridView
            dtHang = dp.DocBang(sql);
            ...
            dgvHang.Columns[3].HeaderText = "Tên Chất liệu";
            ...
            dgvHang.Columns[2].DataPropertyName = "MaChatLieu";
            dgvHang.Columns[3].DataPropertyName = "TenChatLieu";
            dgvHang.Columns[4].DataPropertyName = "DonGiaNhap";
            dgvHang.Columns[5].DataPropertyName = "DonGiaBan";
            dgvHang.Columns[6].DataPropertyName = "SoLuong";
        ...
        private void btnIn_Click(object sender, EventArgs e)
        {
            if (dtHang.Rows.Count > 0) //TH có dữ liệu để ghi
            {
                ...
                for (int i = 0; i < dtHang.Rows.Count; i++)
                {
                    ...
                    exSheet.get_Range("D" + (i + 8).ToString()).Value = dtHang.Rows[i]["TenChatLieu"].ToString();
                    exSheet.get_Range("E" + (i + 8).ToString()).Value = dtHang.Rows[i]["SoLuong"].ToString();
                    exSheet.get_Range("F" + (i + 8).ToString()).Value = dtHang.Rows[i]["DonGiaNhap"].ToString();
                    exSheet.get_Range("G" + (i + 8).ToString()).Value = dtHang.Rows[i]["DonGiaBan"].ToString();
                }
        ...
        {
            DataTable result = (DataTable)dgvHang.DataSource;
            frmBaoCao f = new frmBaoCao(result);
            f.ShowDialog();
        }
    }
}
